...
const AdminUsers = () => {
    ...
    const getUesrs = useCallback(() => {
        setLoading(true)
        fetch(`http://localhost:5000/allUsersInfo/${active}/${time}?show=` + fetchData, {
            method: 'GET',
            headers: {
                authorization: token
            }
    ...
    useEffect(() => {
        const fetchUser = () => {
            setLoading(true)
            fetch('http://localhost:5000/searchUser?search=' + search, {
                method: 'GET',
                headers: {
                    authorization: token
                }
    ...
    const downloadFile = () => {
        ...
        fetch('http://localhost:5000/adminUserDataExport/', {
            headers: {
                authorization: token
            }
        })
    ...
    const banUser = (id) => {
        if (value == '0') {
            setBanLoader(id)
            fetch(`http://localhost:5000/banUser/${id}`, {
                method: 'POST',
                headers: {
                    authorization: token
                }
            ...
            setBanLoader(id)
            fetch(`http://localhost:5000/unBanUser/${id}`, {
                method: 'POST',
                headers: {
                    authorization: token
                }
                        ...
                        <>
                            {query.map(user =>
                                <tr key={user.user_id}>
                                    ...
                                    <td>{user.role}</td>
                                    <td>{user.email}</td>
                                    <td><img src={`http://localhost:5000/${user.avatar}`} alt="userAvatar" className="img-fluid" width="50" height="50" /></td>
