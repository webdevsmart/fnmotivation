...
const AdminStoryCmntReplyReport = () => {
    ...
    const countCommentsReply = useCallback(() => {
        setLoad(true)
        fetch(`http://localhost:5000/countStoryCmntReplyeports`, {
            method: 'GET',
            headers: {
                authorization: token
            }
            ...
            .then(res => res.json())
            .then(data => {
                setActiveComments(data[0].story_comment_reply_report)
                setLoad(false)
            })
    }, [])
    ...
    const getCommentsReply = useCallback(() => {
        setLoading(true)
        fetch(`http://localhost:5000/storyReportCmntReplyDetails/?show=` + fetchData, {
            method: 'GET',
            headers: {
                authorization: token
            }
    ...
    const banStories = (id) => {
        if (value == '0') {
            ...
            fetch(`http://localhost:5000/banReplyComment/${id}`, {
                method: 'POST',
                headers: {
                    authorization: token
                }
            ...
            setBanLoaderComment(id)
            fetch(`http://localhost:5000/unBanReplyComment/${id}`, {
                method: 'POST',
                headers: {
                    authorization: token
                }
        ...
        <section className="m-2">
            ...
            <div className="nav-admin text-center">
                <div className="d-flex justify-content-center">
                    <h3 className="admin-box-inactive bg-danger"><FaCalendarMinus color="white" className="m-2" />Story Comment Reply Reports: {load ? <CircularProgress color="#fbc02d" /> : activeComments} </h3>
                </div>
            </div>
            ...
            <table className="table table-data">
                ...
                <tbody>
                    {commentsReply.map(rep =>
                        <tr key={rep.sc_reply_id}>
                            <td>{rep.story_id}</td>
                            <td>{rep.sc_reply_text}</td>
                            ...
                            <td>
                                {banLoaderComment === rep.sc_reply_id ? '...processing...' :
                                    <div className="dropdown">
                                        <select className="btn btn-outlined-danger" name="status" onClick={(e) => setValue(e.target.value)} onChange={() => banStories(rep.sc_reply_id)}>
...
export default AdminStoryCmntReplyReport;