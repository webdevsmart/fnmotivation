...
const EditStory = () => {
    ...
    const getParticularStory = useCallback(() => {
        ...
        fetch('http://localhost:5000/getParticularStoryWithsUser?id=' + storyID)
            .then(res => res.json())
            .then(data => {
                setStory(data)
    ...
    const onSubmit = async (data) => {
        ...
        fetch('http://localhost:5000/editStory', {
            method: 'POST',
            headers: {
                                ...
                                <form onSubmit={handleSubmit(onSubmit)} onKeyDown={(e) => checkKeyDown(e)}>
                                    {story.map((story, index) =>
                                        ...
                                        {/* <input type="file" size="60" onChange={imageHandler} /> */}
                                                ...
                                                {<span>Can't Update</span>}
                                                {!image.viewImage && story.post_thumbnail ? <img src={`http://localhost:5000/${story.post_thumbnail}`} alt="imageUplaod" className="w-25 mt-2" />
                                                    :
