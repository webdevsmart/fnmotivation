...
const LoginUser = () => {
    ...
    const responseGoogle = (res) => {
        ...
        if (res.profileObj) {
            ...
            fetch(`http://68.183.178.196/api//googleLogin?id=` + res.profileObj.googleId)
                .then(res => res.json())
                .then(data => {
                    if (data.error) {
    ...
    const faceBookLogin = (res) => {
        ...
        if (res.profile) {
            setPreloaderVisibale(true)
            fetch(`http://68.183.178.196/api//facebookLogin?id=` + res.profile.id)
                .then(res => res.json())
                .then(data => {
                    if (data.error) {
                        console.log(data.error)
    ...
    const onSubmit = (data) => {
        ...
        setErrorMessage([])
        setPreloaderVisibale(true)
        fetch('http://68.183.178.196/api//loginUser?email=' + data.email, {
            method: 'GET',
            headers: {
