...
const ShortCommunityStory = ({ artfetchData,community }) => {

    const [load, setLoad] = useState(false)
    const [allArticle, setAllArticle] = useState([])
    const getAllArticle = useCallback(() => {
        setLoad(true)
        fetch(`http://68.183.178.196/api//getCommunityArticle/${community}?show=` + artfetchData)
            .then(res => res.json())
            .then(data => {
...
export default ShortCommunityStory;