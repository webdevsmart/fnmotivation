...
const SearchResultStory = ({ search, location, handleChange, checkKeyDown }) => {
    ...
    const SearchedStory = useCallback(  () => {
        setLoading(true)
        fetch('http://68.183.178.196/api//getSearchedStory?search=' + data)
            .then(res => res.json())
            .then(data => {
                setAllStories(data)
                                                ...
                                                <Link to={"/post/" +story.story_id + "/" + story.title.replace(/\s/g, '-').substring(0, 60)} onClick={scrollToTop}>
                                                    <div className="related-articles-box">
                                                        <div className="image-holder">
                                                            <img src={`http://68.183.178.196/api//${story.post_thumbnail}`} alt="articleImage" className="img-fluid content-image" />
                                                        </div>
