...
const TagStoryPage = () => {
    ...
    const AllStoryWithUser = useCallback(  () => {
        setLoading(true)
        fetch('http://68.183.178.196/api//getTagPosts?tag=' + ids)
            .then(res => res.json())
            .then(data => {
                setAllStories(data)
    ...
    return (
        <div className="related-articles pl-5 pr-5">
            <OgData url={'http://fnmotivation.com/'} title={`Tag - ${str} - FNMotivation`} description={'This new social network is a unique platform that is centered around health and wellness. This platform will provide a central location for people to like-minded people to connect.'} image={'http://68.183.178.196/api//fnmotivation-logo.png'} />
