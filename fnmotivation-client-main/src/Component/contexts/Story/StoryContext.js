...
export const StoryInfoProvider = props => {
    ...
    const ParticularStoryWithUser = useCallback(  () => {
        ...
        fetch('http://68.183.178.196/api//getParticularStoryWithsUser?id=' + id)
            .then(res => res.json())
            .then(data => {
                setStory(data)
                
                const catID = data.map(id => id.community_id)
                fetch('http://68.183.178.196/api//getSelectedStory?catID=' + catID)
                    .then(res => res.json())
                    .then(data => {
    ...
    const allStoryLikes = useCallback(  () => {
        fetch('http://68.183.178.196/api//allStoryLikes?id=' + id)
            .then(res => res.json())
            .then(data => {
                setAllLikes(data)
    ...
    const getStorybookmarks = useCallback(  () => {
        fetch('http://68.183.178.196/api//getStorybookmarks?id=' + idis, {
            method: 'GET',
            headers: {
